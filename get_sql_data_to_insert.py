...
def get_plc_data(
    ...
    for key in fields_dict.keys():
        ...
        # query_engine = DBLoader.get_query_engine(response_schemas, retriever)
        query_engine = DBLoader.get_query_engine(response_schemas, retriever)
        query_str = "For the field {} get all the relevant and detailed information for the product {} based on fields description: {}. Don't use symbols like ':'.".format(
            key,
            product_name,
            fields_dict[key]["description"],
        )
...
def get_plc_embed_data(plc_id, nodes, fields_dict=[]):
    response_device_list = []
